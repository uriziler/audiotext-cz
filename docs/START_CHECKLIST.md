...
- Jaké budou přesné modelové download URL a checksumy?
- Poběží model download přes Android DownloadManager, nebo vlastní HTTP klient?
- Bude `whisper.cpp` vendored source, git submodule, nebo samostatný build artifact?
- Potvrdí se `large-v3-turbo-q5_0` jako MVP Large po měření na S25 Ultra?
